...
object AsyncHttpApi extends Assertions {
  ...
  class NodeExtGrpc(n: Node) {
    ...
    def broadcastIssue(
      ...
      val unsigned = PBTransaction(
        ...
        PBTransaction.Data.Issue(IssueTransactionData.of(
          ...
          decimals,
          reissuable,
          script.map(s => PBScript.of(ByteString.copyFrom(Base64.decode(s)), script.get.getBytes.head)))))
    ...
    def setScript(sender: KeyPair, script: Option[Script], fee: Long, timestamp: Long = System.currentTimeMillis(), version: Int = 1): Future[PBSignedTransaction] = {
      val unsigned = PBTransaction(
        chainId,
        ...
        timestamp,
        version,
        PBTransaction.Data.SetScript(SetScriptTransactionData.of(script.map(PBTransactions.toPBScript))))
    ...
    def broadcastMassTransfer(sender: KeyPair,
      ...
      val unsigned = PBTransaction(
        ...
        PBTransaction.Data.MassTransfer(MassTransferTransactionData.of(
          ...
          transfers,
          if (attachment.isEmpty) None else Some(PBAttachment.of(PBAttachment.Attachment.BinaryValue(attachment)))
        )))
      val proofs = crypto.sign(sender, PBTransactions.vanilla(SignedTransaction(Some(unsigned)), unsafe = true).explicitGet().bodyBytes())
      transactions.broadcast(SignedTransaction.of(Some(unsigned), Seq(ByteString.copyFrom(proofs))))
    }
